...
export default function ButtonGroup({
  ...
  const baseClasses =
    'relative inline-flex items-center px-4 py-2 border text-sm leading-5 font-medium focus:z-10 focus:outline-none focus:border-indigo-300 dark:focus:border-indigo-600 focus:shadow-outline-indigo transition ease-in-out duration-150';
      ...
      {options.map((option, index) => (
        <button
          ...
          className={baseClasses + ' ' +
            ...
            ((index !== 0 && index !== options.length - 1) ? middleButtonClasses : ' ') +
